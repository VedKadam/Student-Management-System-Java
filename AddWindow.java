...
class AddWindow extends JFrame{
	...
	AddWindow(){
		...
		txtrno.addFocusListener(new FocusAdapter() {
  		  public void focusLost(FocusEvent e) {
      			  String input = txtrno.getText().trim();
        		if(!input.matches("[1-9]+")) {
        	    		JOptionPane.showMessageDialog(c, "Rno cannot be kept empty & Rno should be a positive integer only.");
            			txtrno.setText("");
            			txtrno.requestFocus();
        			}
    			}
		...
		ActionListener a2 = (ae)->{
			try{
				...
				String sql = "insert into student(rno, name, submarks1, submarks2, submarks3) values(?, ?, ?, ?, ?)";
				...
				pst.setInt(3, submarks1);
				pst.setInt(4, submarks2);
				pst.setInt(5, submarks3);
			...
			catch(SQLException e)
			{
				JOptionPane.showMessageDialog(c, "record already present");
					...
					JOptionPane.showMessageDialog(c, "Please re-enter marks & Integers Only ");
					txtrno.setText("");
					txtname.setText("");
					txtsubmarks1.setText("");
					txtsubmarks2.setText("");
